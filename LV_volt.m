function msg = LV_volt(msgname,type)
  if(255==type)
  msg = struct;
  msg.num=9;
  msg.list= cell(1, msg.num);
  msg.list{1}='VECTOR__INDEPENDENT_SIG_MSG';
  msg.list{2}='brake_volt';
  msg.list{3}='To_Recorde_pedal2';
  msg.list{4}='error_info';
  msg.list{5}='pedal_velocity';
  msg.list{6}='To_Record_SC';
  msg.list{7}='To_Record_pedal1';
  msg.list{8}='To_Recorder_Battery';
  msg.list{9}='To_Recorder_wheel_velocity';
  else 
  msg = struct;
  switch msgname 
%%
