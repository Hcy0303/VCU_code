function msg = pedal_and_SC(msgname,type)
  if(255==type)
  msg = struct;
  msg.num=8;
  msg.list= cell(1, msg.num);
  msg.list{1}='VECTOR__INDEPENDENT_SIG_MSG';
  msg.list{2}='To_Recorde_pedal2';
  msg.list{3}='error_info';
  msg.list{4}='pedal_velocity';
  msg.list{5}='To_Record_SC';
  msg.list{6}='To_Record_pedal1';
  msg.list{7}='To_Recorder_Battery';
  msg.list{8}='To_Recorder_wheel_velocity';
  else 
  msg = struct;
  switch msgname 
%%
...
case 'To_Record_SC'
    ...
    ECOCAN.To_Record_SC.fields{1}.name = 'R_velocity_aver';
...
case 'To_Recorder_Battery'
    ...
    ECOCAN.To_Recorder_Battery.fields{1}.name = 'total_power';
